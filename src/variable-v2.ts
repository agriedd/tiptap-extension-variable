import { Extension, mergeAttributes, Node, NodeViewProps, NodeViewRenderer } from '@tiptap/core'
...
export type VariableOptions = {
	...
	suggestion: Omit<SuggestionOptions<HTMLElement>, 'editor'>,
	items: any,
}

export const VariableNodeView = (props: Pick<NodeViewProps, 'node'|'getPos'|'editor'>): ReturnType<NodeViewRenderer> =>  {
...
export const Variable = Node.create<VariableOptions>({
	...
	addOptions() {
		return {
			...
			renderHTML({ options, node }) {
				const htmlAttributes = Object.assign(this.HTMLAttributes, { class: 'variable', contenteditable: true })
	...
	addNodeView() {
		return ({ node, getPos, editor }) => {
			const renderView = VariableNodeView({
				node, getPos, editor
			})
			return renderView
		}
	...
	addProseMirrorPlugins() {
		return [
			Suggestion({
				...
				...(typeof this.options.items === 'function' && { items: this.options.items }),
			}),
		]
	},
})
